...
* `baserow/backend:2.1.4` which by default starts the Gunicorn Django backend server 
  for Baserow but is also used to start the celery workers and celery beat services.
* `baserow/web-frontend:2.1.4` which is a Nuxt server providing Server Side rendering 
  for the website.
...
* `baserow/backend:2.1.4` (default command is `gunicorn`)
* `baserow/backend:2.1.4` with command `celery-worker`
* `baserow/backend:2.1.4` with command `celery-export-worker`
* `baserow/web-frontend:2.1.4` (default command is `nuxt-prod`)
* A postgres database 
* A redis server
