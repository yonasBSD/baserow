...
* `baserow/backend:2.1.3` which by default starts the Gunicorn Django backend server 
...
* `baserow/backend:2.1.3` (default command is `gunicorn`)
* `baserow/backend:2.1.3` with command `celery-worker`
* `baserow/backend:2.1.3` with command `celery-export-worker`
* `baserow/web-frontend:2.1.3` (default command is `nuxt-prod`)
* A postgres database 
* A redis server
